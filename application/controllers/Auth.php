...
class Auth extends CI_Controller
{
  ...
  // trim = untuk menghilangkan karakter spasi di database
  ...
  private function _sendEmail($token, $type)
  {
    $config = [
      ...
      'smtp_user' => 'nama.gmail@gmail.com',
      'smtp_pass' => '',
      'smtp_port' => 465,
      'mailtype'  => 'html',
      'charset'   => 'utf-8',
    ...
    $this->email->from('nama.gmail@gmail.com','Nama lengkap');
  ...
  public function verify()
  {
    ...
    if ($user) {
      ...
      if ($user_token) {
        // expired untuk validasi akun

        // waktu saat ini dikurangin waktu akun di buat
        if( time() - $user_token['date_created'] < (60*60*24)){
          ...
          $this->session->set_flashdata('flash','<div class="alert alert-success" role="alert">
          '. $email .' hase been activated! Please login.</div>');
          redirect('auth');
        }else{
  ...
  public function forgotPassword()
  {
    ...
    if($this->form_validation->run() == false){
      ...
      $email = $this->input->post('email');
      $user = $this->db->get_where('user', ['email' => $email, 'is_activate' => 1])->row_array();
  ...
  public function resetPassword()
  {
    ...
    if ($user) {
      ...
      if ($user_token) {
        ...
        $this->session->set_flashdata('flash','<div class="alert alert-danger" role="alert">
        Reset password field! Wrong token.</div>');
        redirect('auth');
      }
      
    } else {
      $this->session->set_flashdata('flash','<div class="alert alert-danger" role="alert">
      Reset password field! Wrong email.</div>');
      redirect('auth');
    }
  ...
  public function changePassword()
  {
    ...
    // agar user tidak sembarang ubah password tanpa email 
    ...
    if ($this->form_validation->run() == false){
      ...
      $email = $this->session->userdata('reset_email'); // jika tidak ada session ini, maka user tidak bisa sembarangn ubah password tanpa email
